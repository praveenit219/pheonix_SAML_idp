...
public class SAMLBuilder {
	...
	public static Assertion buildAssertion(SAMLPrincipal principal, Status status, String entityId) {
		if(null!=principal) {
			...
			return buildAssertionDummy(status, entityId);
		}

	}

	private static Assertion buildAssertionDummy(Status status, String entityId) {
		Assertion assertion = buildSAMLObject(Assertion.class, Assertion.DEFAULT_ELEMENT_NAME);

		if (status.getStatusCode().getValue().equals(StatusCode.SUCCESS_URI)) {
			Subject subject = buildSubject("admin", "urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified", "http://localhost:9090/sp/consumer", "a28chbb7jdg4ifc913725i5d1i57cd");
			assertion.setSubject(subject);
		}
		...
		Audience audience = buildSAMLObject(Audience.class, Audience.DEFAULT_ELEMENT_NAME);
		audience.setAudienceURI("http://mock-sp");
		AudienceRestriction audienceRestriction = buildSAMLObject(AudienceRestriction.class, AudienceRestriction.DEFAULT_ELEMENT_NAME);
		audienceRestriction.getAudiences().add(audience);
		...
		assertion.getAttributeStatements().add(buildAttributeStatement(null));
	...
	public static EncryptedAssertion encryptAssertion(Assertion assertion, Credential signingCredential) {
		EncryptionParameters encryptionParameters = new EncryptionParameters();
		encryptionParameters.setAlgorithm(EncryptionConstants.ALGO_ID_BLOCKCIPHER_AES128);

		KeyEncryptionParameters keyEncryptionParameters = new KeyEncryptionParameters();
		keyEncryptionParameters.setEncryptionCredential(signingCredential);
	...
	private static AttributeStatement buildAttributeStatement(List<SAMLAttribute> attributes) {
		if(null!=attributes) {
			...
			return buildAttributeStatementDummy(attributes); 
		}
		

	}

	private static AttributeStatement buildAttributeStatementDummy(List<SAMLAttribute> attributes) {
		AttributeStatement attributeStatement = buildSAMLObject(AttributeStatement.class, AttributeStatement.DEFAULT_ELEMENT_NAME);

        Attribute attributeUserName = buildSAMLObject(Attribute.class, Attribute.DEFAULT_ELEMENT_NAME);
        ...
        Attribute attributeLevel = buildSAMLObject(Attribute.class, Attribute.DEFAULT_ELEMENT_NAME);
